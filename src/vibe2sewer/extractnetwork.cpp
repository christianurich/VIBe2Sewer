...
VIBe_DECLARE_NODE_NAME(ExtractNetwork, Sewer)
...
ExtractNetwork::ExtractNetwork()
{
    ...
    this->addParameter("ConduitLength", VIBe2::DOUBLE, &this->ConduitLength);
...
std::vector<std::vector<Point> > ExtractNetwork::SimplifyNetwork(std::vector<std::vector<Point> > &points, int PReduction, double offset) {
